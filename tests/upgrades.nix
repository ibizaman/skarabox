...
let
  ...
  templateTest = {
    ...
    graphic=-nographic
    tmpdir=
    startRootDisk2=${toBashBool startRootDisk2}
    startDataPool=${toBashBool startDataPool}
    endRootDisk2=${toBashBool endRootDisk2}
    endDataPool=${toBashBool endDataPool}
    ...
    if [ "$startDataPool" = false ]; then
      sed -i 's-enable = true-enable = false-' ./myskarabox/configuration.nix
    else
      sed -i 's-enable = false-enable = true-' ./myskarabox/configuration.nix
    fi
    if [ "$startRootDisk2" = true ]; then
      sed -i 's-disk2 = null-disk2 = "/dev/nvme1n1"-' ./myskarabox/configuration.nix
    else
      sed -i 's-disk2 = "/dev/nvme1n1"-disk2 = null-' ./myskarabox/configuration.nix
    fi
    ...
    if [ "$endDataPool" = false ]; then
      sed -i 's-enable = true-enable = false-' ./myskarabox/configuration.nix
    else
      sed -i 's-enable = false-enable = true-' ./myskarabox/configuration.nix
    fi
    if [ "$endRootDisk2" = true ]; then
      sed -i 's-disk2 = null-disk2 = "/dev/nvme1n1"-' ./myskarabox/configuration.nix
    else
      sed -i 's-disk2 = "/dev/nvme1n1"-disk2 = null-' ./myskarabox/configuration.nix
    fi
...
in
{
  oneOSnoDataToTwoOSTwoData = templateTest {
    name = "oneOSnoDataToTwoOSTwoData";
    startRootDisk2 = false;
    startDataPool = false;
    endRootDisk2 = true;
    endDataPool = true;
  };

  twoOStwoDataToOneOSnoData = templateTest {
    name = "twoOStwoDataToOneOSnoData";
    startRootDisk2 = true;
    startDataPool = true;
    endRootDisk2 = false;
    endDataPool = false;
